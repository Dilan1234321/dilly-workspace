...
LOGFILE="$WORKSPACE/projects/dilly/autonomous_run.log"
PYTHON_BIN="$WORKSPACE/dilly_venv/bin/python"

echo "$(date) - Starting Dilly specialized school metric optimization..." >> "$LOGFILE"
...
"$PYTHON_BIN" "$WORKSPACE/projects/dilly/retrain_brains.py" >> "$LOGFILE" 2>&1

if [ $? -eq 0 ]; then
    echo "$(date) - SUCCESS: Dilly brains retrained with Specialized School Alpha logic." >> "$LOGFILE"
else
