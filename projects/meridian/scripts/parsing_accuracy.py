...
Run from workspace root:
  ...
  MERIDIAN_USE_LLM=1 OPENAI_API_KEY=... python -m projects.meridian.scripts.parsing_accuracy   # include LLM
...
TRAINING_PATH = os.path.join(WORKSPACE, "projects", "meridian", "prompts", "training_data.json")
...
def evaluate_rule_based(examples: list) -> tuple[dict, list]:
    ...
    from meridian_core.auditor import get_track_from_major_and_text
...
def main():
    ...
    use_llm = os.environ.get("MERIDIAN_USE_LLM", "").strip().lower() in ("1", "true", "yes") and os.environ.get("OPENAI_API_KEY")
    if use_llm:
        ...
        print("\n(Set MERIDIAN_USE_LLM=1 and OPENAI_API_KEY to also evaluate LLM parsing.)")
