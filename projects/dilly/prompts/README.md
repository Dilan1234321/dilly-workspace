...
To use rubrics in the LLM: load the appropriate rubric (by detected track) into the system prompt so the model grades against industry standards, not opinion. (Wiring is in `dilly_core/llm_auditor.py` when implemented.)
...
```bash
pip install -r projects/dilly/scripts/requirements-build.txt   # pypdf + python-docx (required)
python -m projects.dilly.scripts.build_training_data
```

- Runs the **rule-based** Dilly auditor on every **PDF and DOCX** in `RESUME_DIR`.
- Saves **`projects/dilly/prompts/training_data.json`** with resume excerpts and scores/findings.
...
- `DILLY_TRAINING_DATA` — output path (default: `projects/dilly/prompts/training_data.json`).
- `DILLY_EXCERPT_CHARS` — max chars per resume in the file (default: 2400).
- `DILLY_PDF_OCR_THRESHOLD` — try OCR when pypdf returns fewer than this many chars (default: 100).

## 3. Use the LLM with training enabled

- Set **`DILLY_USE_LLM=1`** and **`OPENAI_API_KEY`** so the API uses the LLM.
...
- `DILLY_FEW_SHOT=0` — turn off few-shot (no examples).
- `DILLY_FEW_SHOT_N=4` — use 4 examples instead of 3.
- `DILLY_TRAINING_DATA` — path to a different training JSON.
...
| 2 | `pip install -r projects/dilly/scripts/requirements-build.txt` then run `python -m projects.dilly.scripts.build_training_data`. |
| 3 | Set `DILLY_USE_LLM=1` and `OPENAI_API_KEY`; restart API. |
...
```bash
DILLY_USE_LLM=1 OPENAI_API_KEY=your_key python -m projects.dilly.scripts.parsing_accuracy
```

### Improve until 95%+

1. Run the script; note which fields fail and for which files.
2. **Rule-based:** Fix `dilly_core/resume_parser.py` (e.g. `REJECT_WORDS_IN_NAME`, major keywords, name-extraction logic).
3. **LLM:** Tighten the system prompt in `dilly_core/llm_auditor.py` or add better few-shot examples.
