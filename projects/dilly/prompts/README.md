...
- **`rubric_builder.md`** — Standalone reference doc for recruiter/generalist scoring (legacy Builder track; file kept for reference). Sourced from SHRM, hiring-manager and recruiter surveys (ResumeTemplates, Jobseeker, Jobscan), and quantifiable-achievement research. Use so the LLM follows what employers and recruiters prioritize (communication, professionalism, impact metrics, experience fit).
...
| 2 | `pip install -r projects/meridian/scripts/requirements-build.txt` then run `python -m projects.dilly.scripts.build_training_data`. |
...
```bash
MERIDIAN_USE_LLM=1 OPENAI_API_KEY=your_key python -m projects.dilly.scripts.parsing_accuracy
```
