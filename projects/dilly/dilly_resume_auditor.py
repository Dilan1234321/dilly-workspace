...
class DillyResumeAuditor:
    def __init__(self, file_path):
        ...
        self.raw_text = ""
        self.analysis = {}
        self.page_count = None  # PDF: number of pages; DOCX: None (use word count for length checks)
    ...
    def extract_text(self):
        ...
        if path_lower.endswith(".docx"):
            try:
                ...
                self.page_count = None  # DOCX has no simple page count
        ...
        # 3) OCR fallback (tesseract on rendered page images)
        threshold = int(os.environ.get("DILLY_PDF_OCR_THRESHOLD") or os.environ.get("MERIDIAN_PDF_OCR_THRESHOLD", "100"))
        best_text = ""

        pypdf_text = self._pdf_text_via_pypdf()
        if pypdf_text:
            best_text = pypdf_text

        pymupdf_text = self._pdf_text_via_pymupdf()
        if pymupdf_text and len(pymupdf_text.strip()) > len(best_text.strip()):
            best_text = pymupdf_text

        if len(best_text.strip()) < threshold:
            ocr_text = self._pdf_text_via_ocr()
            if ocr_text and len(ocr_text.strip()) > len(best_text.strip()):
                best_text = ocr_text

        self.raw_text = (best_text or "").strip()
        return bool(self.raw_text)

    def _pdf_text_via_pypdf(self) -> str:
    ...
    def _pdf_text_via_pymupdf(self) -> str:
        if not PYMUPDF_AVAILABLE:
            return ""
        try:
            doc = fitz.open(self.file_path)
            self.page_count = self.page_count or len(doc)
            chunks = []
            for page in doc:
    ...
    def _pdf_text_via_ocr(self) -> str:
        """
        OCR fallback for scanned/image-only PDFs.
        Requires: pymupdf + system `tesseract` binary.
        """
        if not PYMUPDF_AVAILABLE:
            return ""
    ...
    def generate_report(self):
        if not REPORTLAB_AVAILABLE:
            ...
            return None
        filename = f"dilly_resume_audit_{datetime.datetime.now().strftime('%Y%m%d')}.pdf"
...
if __name__ == "__main__":
    ...
    auditor = DillyResumeAuditor(res_path)
