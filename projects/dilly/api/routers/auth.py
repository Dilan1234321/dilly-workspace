...
async def create_billing_portal_session(request: Request):
    """Create a Stripe Customer Portal session for an already-subscribed user. Returns { url }."""
    u = deps.bearer_user(request)
    if not u:
        raise errors.unauthorized("Sign in to manage billing.")
    email = (u.get("email") or "").strip().lower()
    if not email:
        raise errors.validation_error("Email required.")
    if not _stripe_configured():
        return {"url": None, "message": "Payment not configured."}
    base = str(request.base_url).rstrip("/")
    return_url = os.environ.get("STRIPE_CANCEL_URL", base.replace("/api", "") or "/") + "/settings"
    try:
        import stripe
        stripe.api_key = os.environ.get("STRIPE_SECRET_KEY")
        # Look up customer by email — Stripe keeps one customer per email by default
        customers = stripe.Customer.list(email=email, limit=1)
        if not customers.data:
            return {"url": None, "message": "No billing account found. Please contact support."}
        ...
        raise errors.internal("Could not create billing portal session.")
