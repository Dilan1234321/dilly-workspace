...
# Load .env from workspace root when present (e.g. DILLY_USE_LLM, OPENAI_API_KEY)
try:
...
app = FastAPI(title="Dilly AI API")
...
def health():
    ...
    return {"status": "ok", "backend": "Dilly AI API"}
...
async def audit_resume_v2(file: UploadFile = File(...)):
    """Dilly Auditor V2: Ground Truth V6.5 scoring + Vantage Alpha tracks. Evidence-based audit_findings only."""
    if not _allowed_resume_file(file.filename):
        raise HTTPException(status_code=400, detail="Only PDF and DOCX files are supported.")
    ...
    try:
        ...
        raw_logs = [
            ...
            "LLM Auditor (MTS)." if use_llm and os.environ.get("OPENAI_API_KEY") else "Dilly Core V6.5 + Vantage Alpha.",
            recs_source,
        ]
