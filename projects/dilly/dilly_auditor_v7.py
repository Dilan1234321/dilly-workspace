...
class DillyAuditorV7:
    """
    Dilly Auditor V7: True-Omni Format Truth Standard
    ...
    def get_contextual_data(self):
        ...
        tech_sigs = ["python", "sql", "aws", "intern", "developer", "finance", "economics", "business"]
        health_sigs = ["medical", "hospital", "patient", "clinical", "shadowing", "scribe", "emt", "biochem"]

        tech_majors = ["Data Science", "Computer Science", "Finance", "International Business", "Economics"]

        if any(k in text for k in tech_sigs) or major in tech_majors:
            track = "Tech"
        else:
            track = "Pre-Health"

        if any(k in text for k in health_sigs):
            if major not in tech_majors:
                track = "Pre-Health"
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) > 1:
        engine = DillyAuditorV7(sys.argv[1])
