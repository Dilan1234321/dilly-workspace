...
function formatPlaybookHttpError(status: number, bodyText: string): string {
  if (status === 404) {
    return "We couldn't reach the playbook service. It may not be deployed on this API yet. You can still use the Job search checklist on Get Hired.";
  }
...
function buildClientPlaybookWhenApi404(
  ...
  for (const b of bullets.slice(0, 8)) {
    ...
    dives.push({
      theme: t.slice(0, 220),
      for_you: "Personalized coaching for this theme needs the AI service. Use your latest audit recommendations and ask Dilly in chat to apply this to your bullets.",
...
export function CareerPlaybookTab({
  onBack,
  onOpenDilly,
}: {
  onBack: () => void;
                ...
                <ul className="space-y-3 list-none p-0 m-0">
                  {data.resume_signals.map((s, i) => (
                    <li key={i} className="border-t first:border-t-0 first:pt-0 pt-3" style={{ borderColor: "var(--b1)" }}>
                      <p className="text-[13px] font-semibold" style={{ color: "var(--t1)" }}>{s.signal}</p>
                      {s.from_resume ? (
                        <p className="text-xs mt-1.5 pl-3 border-l-2 italic leading-relaxed" style={{ borderColor: "var(--te-gold)", color: "var(--t3)" }}>
                          {s.from_resume}
                        </p>
                      ) : null}
                      <p className="text-xs mt-2 leading-relaxed" style={{ color: "var(--t2)" }}>{s.why}</p>
                    </li>
                  ))}
                </ul>
              </section>
              ...
              <ul className="space-y-4 list-none p-0 m-0">
                {(data.deep_dive ?? []).map((d, i) => (
                  <li key={i} className="rounded-xl p-3" style={{ background: "rgba(255,255,255,0.03)" }}>
                    <p className="text-[13px] font-semibold mb-2" style={{ color: "var(--t1)" }}>{d.theme}</p>
                    {d.for_you ? (
                      <p className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color: "var(--t2)" }}>{d.for_you}</p>
                    ) : null}
                    {d.this_week ? (
