...
type PlayStoreButtonProps = Omit<
  React.ComponentProps<typeof Button>,
  "children"
> & {
  /** When set, renders an anchor that looks like the button (for Play Store link). */
  href?: string;
...
function PlayStoreIcon({
  ...
  return (
    <svg viewBox="0 0 24 24" fill={fill} aria-hidden {...props}>
...
const content = (
  <>
    <PlayStoreIcon className="size-5 shrink-0" />
    <div className="flex flex-col items-start justify-center pr-2 text-left">
      <span className="text-[10px] font-light leading-none tracking-tighter">
        GET IT ON
      </span>
...
export function PlayStoreButton({
  ...
  const baseClass = cn("h-11 gap-2", className);
  ...
  if (href) {
    return (
      <a
        ...
        className={cn(buttonVariants({ variant, size }), baseClass)}
      >
        {content}
      </a>
    );
