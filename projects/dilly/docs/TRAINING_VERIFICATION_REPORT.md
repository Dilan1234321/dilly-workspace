...
**Date:** 2025-03-03  
**Scope:** `training_data.json` (14 examples) vs Dilly scoring logic in `dilly_core` (`scoring.py`, `tracks.py`, `auditor.py`).  
...
- **Verification script:** `projects/dilly/scripts/verify_training_scores.py` runs `run_audit()` on each example (using full PDF text when available and pypdf is installed; otherwise the stored excerpt) and compares Smart, Grit, Build, and Final to `training_data.json`.
...
3. Run:  
   `python3 projects/dilly/scripts/verify_training_scores.py`  
