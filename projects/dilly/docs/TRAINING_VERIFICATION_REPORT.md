...
**Date:** 2025-03-03  
**Scope:** `training_data.json` (14 examples) vs Meridian scoring logic in `dilly_core` (`scoring.py`, `tracks.py`, `auditor.py`).  
...
- **Logic reference:** There is no separate `SCORING_LOGIC.md` in the repo; it was created from the code and saved at workspace root. All formulas there match `dilly_core`.
...
1. **Track reassignment (4 examples)**  
   Bridget Klaus, Shreya Mehta, Thomas Rosenblum, Vir Shah are now classified **Pre-Health** from text (e.g. medical, clinical, shadowing, LECOM, pre-med). In `training_data.json` they are **Humanities**.
   - Pre-Health Build = clinical/research density; Tech Build = tech stack + projects.
...
3. **Example (Deng)**  
   ...
   - From excerpt: leadership_density 2, work_entry_count 5 → Grit 54; Tech Build from tech/projects → 8.  
...
1. **Regenerate training_data from current code (optional):**  
   ...
   `python3 -m projects.dilly.scripts.build_training_data`  
...
2. **Pre-Health vs Humanities in training_data:**  
...
3. **Keep SCORING_LOGIC.md in sync:**  
   When you change formulas in `dilly_core`, update `SCORING_LOGIC.md` so it remains the single written reference for Smart, Grit, Build, and Final.
