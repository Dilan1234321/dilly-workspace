...
class MeridianBatchAuditor:
    ...
    def process_all(self):
        ...
        self.df['meridian_score'] = self.df[col_name].apply(self.score_resume)
        
        # Get stats
        top_performers = self.df.nlargest(10, 'meridian_score')
        avg_score = self.df['meridian_score'].mean()
        
        summary = {
            "total_processed": len(self.df),
            "average_grit_score": round(avg_score, 2),
            "top_categories": self.df.groupby('Category')['meridian_score'].mean().nlargest(5).to_dict()
        }
...
if __name__ == "__main__":
    auditor = MeridianBatchAuditor("assets/Resume.csv")
