#!/usr/bin/env python3
"""
Run the Dilly job scraper. Ethical/legal sources only.
...
Run from WORKSPACE ROOT (the folder that contains projects/):
  ...
  python3 projects/dilly/scripts/run_job_scraper.py --limit 5
...
# Add workspace root for imports (workspace/projects/dilly/scripts -> workspace)
...
def main():
    ...
    parser.add_argument("--db", type=str, help="SQLite DB path (default: projects/dilly/dilly_jobs.db)")
    args = parser.parse_args()

    db_path = Path(args.db) if args.db else _SCRIPT_DIR.parent / "dilly_jobs.db"
    run_job_scraper(
        db_path=db_path,
