...
Optional env:
  DILLY_PDF_OCR_THRESHOLD=100  — below this many chars from pypdf we try OCR (default 100). OCR needs: pip install pdf2image pytesseract; system: poppler, tesseract-ocr.
...
OUTPUT_PATH = os.environ.get("DILLY_TRAINING_DATA")
if not OUTPUT_PATH:
...
EXCERPT_CHARS = int(os.environ.get("DILLY_EXCERPT_CHARS", "2400"))
# Min chars from native PDF extraction below which we try OCR (when pdf2image + pytesseract available)
PDF_OCR_THRESHOLD = int(os.environ.get("DILLY_PDF_OCR_THRESHOLD", "100"))
...
def main():
    ...
    print("The LLM auditor will use these as few-shot examples when DILLY_FEW_SHOT=1 (or default).")
