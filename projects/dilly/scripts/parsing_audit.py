...
Usage (from workspace root):
  python projects/dilly/scripts/parsing_audit.py [--sources DIR] [--out REPORT.md]
...
# So we can import dilly_resume_auditor
...
def extract_raw_text(path: str) -> str:
    """Extract plain text from PDF or DOCX using DillyResumeAuditor."""
    try:
...
def run_audit(sources_dir: str, out_path: str) -> None:
    ...
    lines = [
        ...
        "",
        "Generated by `projects/dilly/scripts/parsing_audit.py`. Compare parsed output to source docs per `docs/PARSING_ACCURACY_SPEC.md`.",
        "",
        "## Summary",
        "",
