...
def _classify_job_type(listing: Dict[str, Any]) -> str:
    ...
    # Honor explicit value when source supplied a meaningful one.
    valid = {"internship", "research_internship", "entry_level",
             "full_time", "part_time", "senior", "other"}
    if explicit in valid and explicit != "":
        # But still re-check senior signals: an "internship"-tagged role
        # whose title is "Senior Software Engineer" is misclassified at
        # source. Trust the title in that conflict.
        if explicit in ("internship", "research_internship", "entry_level"):
            if _SENIOR_RE.search(title):
                return "senior"
        return explicit
    # No explicit hint: derive from title.
...
def ensure_niche_sources_populated(conn, cohort_ids: Optional[Iterable[str]] = None) -> bool:
    ...
    # Floor: <500 active internships across all sources is "sparse".
    # The cron-fed steady state should sit in the thousands; below 500
    # almost always means the cron is failing or the deploy is fresh.
