...
// ─── Static role fallbacks (used if API is slow or fails) ────────────────────────
...
export default function BlindAudition() {
  ...
  const { data: rolesData } = useQuery<{ roles: RolePreset[] }>({
    ...
    // If this fails, we fall back to STATIC_ROLES below
    retry: 1,
  });

  // Use API roles if loaded, else static fallback — roles screen never blocks on network
