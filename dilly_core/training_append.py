...
MAX_EXCERPT_CHARS = int(os.environ.get("DILLY_EXCERPT_CHARS", "2400"))
MAX_EXAMPLES = int(os.environ.get("DILLY_TRAINING_MAX_EXAMPLES", "500"))
...
def append_audit_to_training(
    ...
    """
    Append one audit (resume excerpt + scores + evidence) to training_data.json.
    Returns True if append succeeded, False otherwise. Does not raise.
    """
    ...
    example = {
        ...
        "track": getattr(result, "track", "Builder"),
    ...
    try:
        ...
        logger.info("Dilly: appended 1 audit to training_data (%s)", path)
        return True
