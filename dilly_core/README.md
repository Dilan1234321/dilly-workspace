# Dilly Core — Ground Truth V6.5 & Vantage Alpha

Canonical scoring and track logic for the Dilly Auditor. **Zero hallucination:** only evidence present in the resume affects scores.
...
- **`llm_auditor.py`** — **LLM-based pipeline:** `run_audit_llm(...)` → same `AuditorResult`. Uses OpenAI-compatible API; MTS enforced via prompt (evidence-only). Set `DILLY_USE_LLM=1` and `OPENAI_API_KEY` to use.
...
# Requires OPENAI_API_KEY (and optionally DILLY_LLM_MODEL, OPENAI_BASE_URL)
...
- **POST /audit/v2** — PDF or DOCX upload → audit → `AuditResponseV2`. If `DILLY_USE_LLM=1` and `OPENAI_API_KEY` is set, uses LLM; else rule-based.

## Dashboard

- **Next.js** app in `projects/dilly/dashboard`: Premium Medical dark theme, PDF upload, Radar chart (Smart/Grit/Build), Audit Findings feed. Uses `/audit/v2`.
