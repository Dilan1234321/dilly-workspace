...
def _build_system_prompt(baseline_smart: int, baseline_grit: int, baseline_build: int, track: str) -> str:
    ...
    return f"""You are Dilly's job-readiness expert. Dilly scores candidates on three dimensions (0–100 each): Smart, Grit, Build. Your task is to infer the **minimum** scores for a **strong fit** for this role.
...
def jd_to_dilly_scores(
    ...
    *,
    ...
    """
    Infer Dilly score requirements from a job description.
...
def jd_to_required_scores_for_job(
    ...
    *,
    ...
    """
    Same as jd_to_dilly_scores but returns a shape suitable for
