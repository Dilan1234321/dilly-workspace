"""
Dilly LLM Auditor — LLM-based scoring with Dilly Truth Standard (MTS).
...
from dilly_core.auditor import AuditorResult, get_track_from_major_and_text, name_from_filename
...
SYSTEM_PROMPT = """You are the Dilly Auditor: a strict, evidence-only resume scoring system. You follow the Dilly Truth Standard (MTS).
...
def _find_training_data_path() -> str | None:
    ...
    env_path = os.environ.get("DILLY_TRAINING_DATA")
...
def _call_llm(resume_text: str, few_shot_block: str | None = None) -> str:
    ...
    model = os.environ.get("DILLY_LLM_MODEL", "gpt-4o-mini")
...
def run_audit_llm(
    ...
    *,
    ...
    try:
        few_shot_block = ""
        if os.environ.get("DILLY_FEW_SHOT", "1").strip().lower() in ("1", "true", "yes"):
            examples = _load_few_shot_examples(
                max_examples=int(os.environ.get("DILLY_FEW_SHOT_N", "3")),
                max_chars_per_excerpt=int(os.environ.get("DILLY_EXCERPT_CHARS", "2400")),
            )
